...
export class EnrollmentQueryService {
   ...
   getAll(): Observable<Enrollment[]> {
      return collectionData(this.enrollmentsCollection, { idField: 'id' }).pipe(
         map((enrollments) =>
            [...enrollments].sort((a: any, b: any) => b.createdAt?.toMillis() - a.createdAt?.toMillis())
         )
   ...
   getByStudent(studentId: string): Observable<Enrollment[]> {
      const q = query(this.enrollmentsCollection, where('studentId', '==', studentId), orderBy('startDate', 'desc'))

      return collectionData(q, { idField: 'id' }) as Observable<Enrollment[]>
   }

   getActiveByStudent(studentId: string): Observable<Enrollment[]> {
      const q = query(this.enrollmentsCollection, where('studentId', '==', studentId), where('status', '==', 'activa'))

      return collectionData(q, { idField: 'id' }) as Observable<Enrollment[]>
   }

   getByBranch(branchId: string): Observable<Enrollment[]> {
      const q = query(this.enrollmentsCollection, where('branchId', '==', branchId), where('status', '==', 'activa'))

      return collectionData(q, { idField: 'id' }).pipe(
         map((enrollments) => [...enrollments].sort((a: any, b: any) => a.studentName.localeCompare(b.studentName)))
      ) as Observable<Enrollment[]>
   }
   ...
   getByBranchAndDateRange(branchId: string, startDate: Date, endDate: Date): Observable<Enrollment[]> {
      const q = query(
         this.enrollmentsCollection,
         where('branchId', '==', branchId),
         where('status', 'in', ['activa', 'completada', 'vencida']), 
   ...
   getExpiredActive(today: Date): Observable<Enrollment[]> {
      const q = query(
         this.enrollmentsCollection,
         where('status', '==', 'activa'),
         where('endDate', '<', FirestoreTimestamp.fromDate(today))
      )
      return collectionData(q, { idField: 'id' }) as Observable<Enrollment[]>
   }
}
