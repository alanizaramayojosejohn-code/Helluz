...
export default class InstructorAttendanceMarkComponent {
   ...
   async onSubmit(): Promise<void> {
      ...
      try {
         const result = await this.attendanceService.markArrival(this.attendanceForm.value)

         const statusIcon = result.isLate ? '⚠️' : '✅'
         const statusText = result.isLate ? `Llegaste ${result.minutesLate} minutos tarde` : 'Llegaste a tiempo'
