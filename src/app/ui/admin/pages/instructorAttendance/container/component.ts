...
export default class InstructorAttendanceListComponent implements OnInit {
   ...
   exportToExcel(): void {
      this.filteredAttendances$.pipe(take(1)).subscribe((attendances) => {
         this.excelExport.exportAsistenciaInstructores(attendances, this.selectedDate$.value)
      })
   }
         ...
         <form [formGroup]="form" class="py-4">
            <div class="mb-4">
               <p class="text-sm text-gray-600"
                  >Instructor: <strong>{{ data.instructorName }}</strong></p
               >
               <p class="text-sm text-gray-600"
                  >Horario: <strong>{{ data.expectedStartTime }} - {{ data.expectedEndTime }}</strong></p
               >
            </div>
